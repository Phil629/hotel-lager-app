...
import { BarChart3, TrendingDown, Package, LayoutGrid, Info, Building2, ChevronDown, ChevronRight, Euro, TrendingUp, Wallet, Calendar, AlertCircle } from 'lucide-react';
                                        ...
                                        {selectedProductData.priceChartData.length > 0 ? (
                                            <ResponsiveContainer width="100%" height="100%">
                                                <LineChart data={selectedProductData.priceChartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                                                    ...
                                                    <Tooltip 
                                                        contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                                                        formatter={(value: number) => [value.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' }), 'Preis']}
...
console.log('done rewrite stats');
