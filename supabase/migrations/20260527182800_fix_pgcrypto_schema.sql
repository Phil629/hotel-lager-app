...
BEGIN
  ...
  IF p_password IS NOT NULL AND p_password <> '' THEN
    ...
    SET encrypted_password = extensions.pgp_sym_encrypt(p_password, v_company_id::text || v_salt)
    WHERE id = v_cred_id;
  END IF;

  IF p_totp_secret IS NOT NULL AND p_totp_secret <> '' THEN
    UPDATE user_supplier_credentials 
    SET encrypted_totp = extensions.pgp_sym_encrypt(p_totp_secret, v_company_id::text || v_salt)
    WHERE id = v_cred_id;
  END IF;
END;
$$;
...
BEGIN
  ...
  IF v_rec.encrypted_password IS NOT NULL THEN
    v_password := extensions.pgp_sym_decrypt(v_rec.encrypted_password::bytea, v_company_id::text || v_salt);
  END IF;

  IF v_rec.encrypted_totp IS NOT NULL THEN
    v_totp := extensions.pgp_sym_decrypt(v_rec.encrypted_totp::bytea, v_company_id::text || v_salt);
  END IF;
