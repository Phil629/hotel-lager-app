...
BEGIN
  ...
  IF p_password IS NOT NULL AND p_password <> '' THEN
    ...
    SET encrypted_password = encode(extensions.pgp_sym_encrypt(p_password, v_company_id::text || v_salt), 'base64')
    WHERE id = v_cred_id;
  END IF;

  IF p_totp_secret IS NOT NULL AND p_totp_secret <> '' THEN
    UPDATE user_supplier_credentials 
    SET encrypted_totp = encode(extensions.pgp_sym_encrypt(p_totp_secret, v_company_id::text || v_salt), 'base64')
    WHERE id = v_cred_id;
  END IF;
END;
$$;
...
BEGIN
  ...
  IF v_rec.encrypted_totp IS NOT NULL THEN
    BEGIN
      v_totp := extensions.pgp_sym_decrypt(decode(v_rec.encrypted_totp, 'base64'), v_company_id::text || v_salt);
    EXCEPTION WHEN OTHERS THEN
      v_totp := NULL;
    END;
  END IF;
